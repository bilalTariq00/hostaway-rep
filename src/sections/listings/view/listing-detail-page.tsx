...
const mockListingDetail = {
  ...
  amenities: [
    "Wifi", "TV", "Kitchen", "Heating", "Air conditioning", "Washer", "Dryer", 
    "Free parking on premises", "Pool", "Hot tub", "Gym", "Indoor fireplace", 
    "Smoking allowed", "Pets allowed", "Events allowed"
  ],
  basePrice: "€100.00",
...
export default function ListingDetailPage() {
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          {hasEdit && (
            <Button 
              variant="outlined" 
              size="small"
              onClick={() => handleEdit(title.toLowerCase())}
            >
              Edit
            </Button>
          )}
      ...
      {mockListingDetail.amenities.map((amenity, index) => (
        <Chip 
          ...
          icon={
            ...
            amenity === 'Kitchen' ? <Coffee size={16} /> :
            amenity === 'Heating' ? <Flame size={16} /> :
            amenity === 'Air conditioning' ? <Wind size={16} /> :
            amenity === 'Pool' ? <Waves size={16} /> :
            amenity === 'Gym' ? <Dumbbell size={16} /> :
            amenity === 'Smoking allowed' ? <Cigarette size={16} /> :
            amenity === 'Pets allowed' ? <Heart size={16} /> :
            amenity === 'Events allowed' ? <PartyPopper size={16} /> :
            <CheckCircle size={16} />
          }
        />
      ))}
  ...
  const renderPricingSection = () => (
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
        <Typography variant="body1">Long term discounts: {mockListingDetail.longTermDiscounts}</Typography>
        <Box>
          <Button size="small" variant="outlined" sx={{ mr: 1 }}>Edit</Button>
          <Button size="small" variant="outlined" sx={{ mr: 1 }}>View</Button>
          <Button size="small" color="error" variant="outlined">Delete</Button>
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Typography variant="h6" sx={{ mb: 2 }}>About this listing</Typography>
      ...
      {renderSectionContent("The space", (
        <Box>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
            <Typography variant="body1">Free cancellation: {mockListingDetail.freeCancellation ? 'Yes' : 'No'}</Typography>
          </Box>
        </Box>
      ))}
      ...
      {renderSectionContent("Availability", (
        <Box>
          <Typography variant="body1" sx={{ mb: 2 }}>All days are available</Typography>
          <Typography variant="body1">Blocked dates</Typography>
        </Box>
      ))}

      {renderSectionContent("Photos", (
      ...
      {renderSectionContent("Policies", (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
            <Typography variant="body1">Cancellation policy: {mockListingDetail.cancellationPolicy}</Typography>
          </Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
            <Typography variant="body1">House rules: {mockListingDetail.houseRules}</Typography>
          </Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
            <Typography variant="body1">Guest requirements: {mockListingDetail.guestRequirements}</Typography>
          </Box>
        </Box>
      ))}

      {renderSectionContent("Booking settings", (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
            <Typography variant="body1">Instant booking: {mockListingDetail.instantBooking ? 'On' : 'Off'}</Typography>
          </Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
      ...
      {renderSectionContent("Payout settings", (
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
            <Typography variant="body1">Payout method: {mockListingDetail.payoutMethod}</Typography>
          </Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
            <Typography variant="body1">Payout currency: {mockListingDetail.payoutCurrency}</Typography>
          </Box>
        </Box>
      ))}

      {renderSectionContent("Cancellation policy", (
        <Box>
          <Typography variant="body1" sx={{ mb: 1 }}>{mockListingDetail.cancellationPolicy}</Typography>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Box sx={{ height: 300, bgcolor: 'grey.200', borderRadius: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Typography variant="body1" color="text.secondary">Map View</Typography>
          </Box>
        </CardContent>
      </Card>
      ...
      <Dialog open={editModalOpen} onClose={handleCloseEdit} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Typography variant="body1">
            Edit form for {editingSection} would go here...
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleCloseEdit}>Cancel</Button>
          <Button variant="contained" onClick={handleCloseEdit}>Save</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
